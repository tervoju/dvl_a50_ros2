...
# constants: in practise these has to be requested from location service
...
class aquatec_ctd(Node):
    def __init__(self):
        super().__init__('aquatec_ctd')
        self.publisher_ = self.publisher()
        self.i = 0

    def read_ctd_values(self, ctd_response):
    ...
    def publisher(self):
        pub = self.create_publisher(DVL, 'ctd/data', 10)
	
        while True:
            response = send_ctd()
            print ("response:", binascii.hexlify(bytearray(response)))
            res,temperature, pressure, conductivity = read_ctd_values(response)
            if res != -1:
...
def main():
    ...
    ctd_publisher = aquatec_ctd()
    rclpy.spin(aquatec_ctd)
	#ctd_publisher.destroy_node()
    rclpy.shutdown()
